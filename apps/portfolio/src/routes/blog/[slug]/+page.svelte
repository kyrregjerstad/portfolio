<script lang="ts">
	...
	const canonicalUrl = $derived(`https://kyrre.dev/blog/${data.meta.slug}`);
	const keywords = $derived(data.meta.seoKeywords || data.meta.categories);
	const pageTitle = $derived(data.meta.seoTitle || data.meta.title);
	const description = $derived(
		data.meta.seoDescription || data.meta.description || `Read ${data.meta.title} by ${data.meta.author}`
	);
	const ogImage = $derived(data.meta.ogImage || 'https://kyrre.dev/images/og-kyrre-gjerstad-blog.jpg');
</script>
...
<Divider />
<!-- <Comments slug={data.meta.slug} isLoggedIn={data.isLoggedIn} /> -->
